...
|---------------------|----------|------------------|----------|---------|------------------------------------------------------------------|
| imageInput          | input    | Multiple         | No       | (empty) | Image source (base64 data URL). Leave blank for empty state; use drag-drop or host binding to set. |
...
| isImageLoaded      | TwoOptions (Boolean)| True when a real image is loaded (host imageInput or successful drag-drop). False when cleared via clearToken. |
| droppedImageBase64 | SingleLine.Text     | Base64 data URL of the last image dropped onto the control. Use with Drop token for app sync. |
| dropToken          | Whole Number        | Increments each time a new image is dropped. Compare with a variable to detect new drops. |

## Using imageInput, drag-drop, clearToken, and isImageLoaded

The control keeps an authoritative **image loaded** state and supports an **empty state** plus **drag-and-drop**: you can start with no image and drop one in, or bind **imageInput** to a variable (e.g. from Add picture).

### Recommended app variables

- **varBase64** (Text) – for host-provided images (e.g. Add picture). Leave blank for empty state. Drag-drop loads internally and can also set the image.
- **varClearToken** (Number) – increment to clear/reset the control (e.g. Clear button: `Set(varClearToken, varClearToken + 1)`).
- **varLastDropToken** (Number) – optional; last **Drop token** value seen for syncing **droppedImageBase64** into the app.

### Bindings

- **ImageCrop1.imageInput** = `varBase64`
- **ImageCrop1.clearToken** = `varClearToken`
- **ImageCrop1.enableDragDropLoad** = `true` when you want drag-and-drop.

### Clear button (reliable reset)

Clearing happens **only** when **clearToken** changes. Drag-dropped images persist until you explicitly clear.

- **Clear button OnSelect:**  
  `Set(varBase64, Blank());`  
  `Set(varClearToken, varClearToken + 1);`

The control clears fully; **isImageLoaded** becomes false.

### Reselecting the same image after clear

If you use **Add picture** (or similar) and then clear, selecting the **same** file again may not reload in the control. In Power Apps, use **Reset(Control)** on the Add picture control (or clear and set the image variable again) so the host sends a fresh image; the PCF will then load it correctly.

### Host image and drag-drop

When **imageInput** (e.g. varBase64) is set to a valid image data URL, the control loads it. Drag-drop loads internally; the dropped image is shown until the host sends a **new** imageInput or you increment **clearToken**. A new host image (e.g. new varBase64 from Add picture) reclaims the display from a previously dropped image.

### Enable/disable Clear button

- **Clear button DisplayMode:** `If(ImageCrop1.isImageLoaded, DisplayMode.Edit, DisplayMode.Disabled)`

### Drop sync (Timer pattern)

When the user drops an image, the control sets **Dropped image (Base64)** and increments **Drop token**. To copy the dropped image into your app (e.g. on a Timer or OnVisible):

- **Timer** (e.g. every 1 second) **OnTimerEnd** (or similar):
...
| **No**      | ΓÇö                | Enabled (normal behavior)  |
...
- `gblEnableCropping`, `gblEnablePan`, `gblUseShiftPan` ΓÇö all default to `false` if not set.
...
- **Display name rename (Eurasia Image Crop)**: The user-facing name was changed from ΓÇ£RAW! Image CropΓÇ¥ to **Eurasia Image Crop**. Only display strings were updated; namespace, constructor, and bindings are unchanged.
...
- **Cover-fit not applied for some images**: The ΓÇ£resizeΓÇ¥ branch (preserve zoom) was running when the **image** changed as well as when the viewport resized. We now only run that branch when the **viewport** dimensions change; when natural size changes (new image or first load), we always run initial cover-fit. `lastNaturalRef` tracks natural dimensions so a new image always gets `baseScale = max(viewportW/imgW, viewportH/imgH)` and centered, clamped translation.
- **Coordinate space**: Pan/zoom transform is in **viewport space** (container pixels). Crop state is in **displayed media space** (react-image-cropΓÇÖs PixelCrop). We did not change how crop output is produced; `useCropToBase64` still uses the libraryΓÇÖs completedCrop and image dimensions. No existing inputs/outputs or crop semantics were changed.

### Crop box visibility and cover-fit (follow-up)

- **Why the crop box disappeared**: With pan/zoom, the media (image) was given layout size `naturalWidth`├ù`naturalHeight` and a CSS transform. ReactCrop sizes its overlay from the mediaΓÇÖs layout size, so the overlay was drawn in a huge (e.g. 1920├ù1080) coordinate space while the visible viewport was only 450├ù350; the crop outline and mask were effectively off-screen or at the wrong scale. In addition, `crop` could be `undefined` when no default was set, so the library sometimes had nothing to draw.
...
- **Verified**: 450├ù350 viewport fills with no empty space for wide, tall, and square images; aspect ratio is preserved (uniform scale only). Crop box outline and shaded highlight are visible and aligned; crop can be moved and resized when Shift is not held; Shift still disables crop interactions only while held.
- **Coordinate space**: With viewport-sized media, the libraryΓÇÖs `onComplete` PixelCrop is in viewport pixels; we convert to image pixels for `completedCrop` so existing crop outputs and semantics are unchanged.

### Crop mouse tracking and zoom-out

- **cropRect coordinate space**: When pan/zoom is active, **crop is stored and passed in viewport space (px)**. The media is a viewport-sized div, so the libraryΓÇÖs hit-testing, drag, and resize all use the same viewport coordinates; no offset. Crop is created only when the user enters crop mode (e.g. double-click) or when showCropByDefault is true (legacy); default rect is centered 70% or defaultCrop converted to viewport px. `onChange` from the library updates crop (viewport px); we mark it so we donΓÇÖt re-convert. Export still converts viewport ΓåÆ image in `onComplete` for `useCropToBase64`.
- **Pointer mapping**: The library receives pointer events on the overlay; it uses the mediaΓÇÖs `getBoundingClientRect()` (the viewport-sized div), so `clientX - rect.left` / `clientY - rect.top` are viewport px. We donΓÇÖt do our own pointerΓåÆcrop math; we just ensure the crop we pass is in viewport px so the libraryΓÇÖs internal drag/resize math matches.
- **Why zoom-out was broken**: Wheel used a fixed step (0.9 / 1.1). Zoom-out (scale ├ù 0.9) was correct in code; the real issue was ensuring **minScale is always the cover scale** (never current scale) and that both directions apply. We use `Math.exp(-e.deltaY * 0.001)` so scroll down = zoom out, scroll up = zoom in; no early-return for deltaY. `zoomAt` clamps to `[minScale, maxScale]`; zoom-out stops at minScale so the viewport stays filled.
...
- **What fixed it**: (1) Run initial cover-fit only when dimensions actually changed: `else if (naturalChanged || isFirstRun)` so we never overwrite scale after user zoom. (2) Set `maxScale = minScale * MAX_SCALE_MULTIPLE` (8├ù) with no Math.max so maxScale is always strictly greater than minScale.
- **maxScale now**: Zoom range is 0.4├ùΓÇô3.0├ù of baseScale (see below).

### Zoom range (0.4├ùΓÇô3.0├ù of baseScale)
...
- **Coordinate spaces and export**: (A) **Viewport space**: origin top-left of control, units = CSS pixels; crop rect is in this space. (B) **Image natural space**: origin top-left of source bitmap, units = naturalWidth ├ù naturalHeight. (C) **View transform**: image ΓåÆ viewport is `vx = ix*scale + translateX`, `vy = iy*scale + translateY`. Export is **viewport-based**: `ix = (vx - translateX)/scale`, `iy = (vy - translateY)/scale`, `iw = vw/scale`, `ih = vh/scale`, . Output canvas = crop window; drawImage at (tx - cropX, ty - cropY) with scaled size; same `scale`, `translateX`, `translateY` used for rendering are used for export (no ΓÇ£centeredΓÇ¥ or baseScale-only assumption).
- **Viewport = control**: The crop overlay viewport is always the full control (e.g. 450├ù350). Viewport rect is `vx=0, vy=0, vw=viewportW, vh=viewportH`. The media passed to ReactCrop is a viewport-sized div (same size as the control), so crop rect and overlay are in **viewport (control) coordinates**; they do not shrink with the image.
...
- **Wheel**: Handler attached to the control container with `addEventListener("wheel", ..., { passive: false })`; `preventDefault()` always called so the page doesnΓÇÖt scroll.
...
- **Power Apps binding (Option A ΓÇö simplest)**  
  ...
  If the slider doesnΓÇÖt update when Default changes while interacting, use Option B.
- **Power Apps binding (Option B ΓÇö robust)**  
